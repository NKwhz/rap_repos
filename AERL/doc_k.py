...
def doc2vec(k, alpha=0, beta=0, activation=activ, use_bias=False, latent_dim=0, epochs=5, batch_size=500000,
            test_doc_fname=test_doc):
    # recall@k

    x_train_doc = np.load(train_doc.format(k))  # doc2vec feature file
    x_train0 = np.load(train_pho0)
    x_train1 = np.load(train_pho1)
    y_train = np.load(train_y)

    x_test_doc = np.load(test_doc.format(k))  # doc2vec feature file
...
def VaeRL2(k, alpha=1.0, beta=1.0, activation=activ, use_bias=True, epochs=5, batch_size=1000,
...
def func(k, model, name, epoch, log_f, turns, dim, batch_size):
    r = []
    log_f.write('\n======{}======\n'.format(name))
    for j in range(turns):
        result, m_f = model(k, alpha=10, beta=10, epochs=epoch, latent_dim=dim, batch_size=batch_size)
...
def cross_val(model, name, log_f, turns, dim, activs, optims):
    global activ
    global optim
    ...
    for opt in optims:
        for a in activs:
            ...
            for j in range(turns):
                activ = a
                optim = opt
                result, m_f = model(alpha=1, beta=1, epochs=10, latent_dim=dim)
                line = '{}:> {}\n'.format(j, ' '.join([str(f1) for f1 in result]))
                log_f.write(line)
                r.append(result)
                print(a, opt, result)
...
if __name__ == '__main__':
    ...
    names = ['doc2vec', 'rhyme2vec', 'con', 'attention', 'conAE', 'conVAE', 'k discuss', 'AttAE', 'c-line', 'skip-line',
             'DA']
    turn = 5
    batch_sizes = 711240
    res = []
